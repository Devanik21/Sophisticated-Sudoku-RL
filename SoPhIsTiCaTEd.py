...
- 🧠 **Deep Q-Network (DQN)** - Experience replay with prioritized sampling
...
with col_b:
    if st.button("🏋️ Train Agent", use_container_width=True):
...
if st.session_state.get('training_active'):
    st.subheader("🏋️ Training AlphaZero Agent")
...
if len(agent.policy_table) > 50:
    ...
    st.info("🏋️ Train the agent first to unlock Human vs AI mode!")
